...
Rules (docs/specs/seed-atom-conformance.md):
  * one batch per generation with a merkle_root and a non-empty leaf set
  ...
  * strict peg check: sum(new_count) must equal count(reflow.atoms); refuse to
    emit otherwise (--force to override after diagnosis)
...
def main() -> int:
    ...
    cur.execute("select count(*) from reflow.atoms")
    distinct_atoms = cur.fetchone()[0]

    cur.execute(
        """
        select id, encode(merkle_root,'hex'), atoms_added, atoms_converged,
               extract(epoch from created_at)::bigint
        from reflow.generations
    ...
    for gen_id, root_hex, added, converged, created_epoch in rows:
        added = added or 0
        ...
        b = {
            ...
            "kind": args.kind,
            "source_ref": f"reflow:gen:{gen_id}",
            "data_type": args.data_type,
            "height": "0",
            "committed_at": str(created_epoch),
        }
        batches.append(b)
        mapping.append((gen_id, batch_id, first, added))
        if added > 0:
            next_seed += added
            minted += added
    ...
    with open(map_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["gen_id", "batch_id", "first_seed_id", "new_count"])
        w.writerows(mapping)

    conv_only = sum(1 for m in mapping if m[3] == 0)
