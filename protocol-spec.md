...
**Content provenance for non-text artifacts (COMMITTED, not yet built — owner 2026-07-16; carded)**: for images, audio, video, and rich documents, the protocol will integrate with C2PA (Coalition for Content Provenance and Authenticity). C2PA metadata is embedded at capture or creation time — Leica and Nikon ship cameras that do this; Adobe and OpenAI services tag generated content; BBC News tags its images. When such an artifact enters the wallet, its C2PA provenance becomes part of its on-chain record, and the contributor's signature is linked to the manifest. This lets the protocol distinguish authentic human contribution from synthetic generation natively, which matters increasingly as generative output proliferates.
...
- **2026-07-16 — DT-21 canon reconciliation (drift paydown, phase 1).** The conformance comb (`docs/specs/comb-spec-vs-chain.md`, primary-verified) found the build and the documents of record had drifted through four mechanisms: constraint-driven substitutions never surfaced for ratification, sub-spec decisions never flowed up to canon, unlabeled multi-writer documents, and specs living away from their code. This entry records the clear-path repairs: **(1) the √ review window is ratified into canon** — τ(M) = base·√(M/threshold), threshold 4.0 interim (owner-decided 2026-07-10 in `x-reputation-p2-review-windows.md` for consensus determinism; protocol-spec and parameters.md had kept the log curve); **(2) parameters.md v0.8.0** registers every value the build actually runs (s_max 10, type weights incl. Use 0.5, coattestor_weight 0.25, attest_bet_scale 0.1, λ_endorsement 0.08, partial-outcome 0.5, max_coattestors 64, citation_uplift_λ 1.0, mintable_kinds, seeds bounds) — **marked INTERIM: registration is not ratification**, and these are the backtest's first sensitivity targets; **(3) the zero floor is debtless for real** (reversal negations floored + running floor in both read paths — commit `4c69cc6`); **(4) the peg burn hole is being closed by governance** (proposal: burn_vote_veto/prevote/quorum burns off — deposits always return; the chain's first live param-change proposal); **(5) seven spec-internal drift spots fixed** in place (staking-token body text, block-reward circulation line, stablecoin payments line, 1:1 pricing, 30K toll example, 1.5% tax, 2×-heading), plus stale `launch-readiness.md` / `deploy/anchord.service`. **Ratified 2026-07-16 (owner): the value-creation tax is levied at SALE, producer-side, never at issuance** — issuance stays free (free entry; the observation layer must never be tolled), the fund is financed where value is realized; §Economics Flow 3 amended, build conforms as-is. **Group 3 walked and ruled 2026-07-16 (owner) — the resolutions:**
- **2026-07-16 — Standing starts at ZERO; baseline is earned by verification.** Default standing for an unknown address is 0 (the cred ladder's bottom rung, now literal); `baseline_kyc` is GRANTED through a verification/ratification mechanism (a verified-identity set, authority-managed at v0; the identity layer is its durable home). Until the chain change lands (first governed upgrade), the build's everyone-gets-baseline interim stands, bounded by the fact that only treasury-funded accounts can act at all.
- **2026-07-16 — Access pricing is a CONSTANT: one photon per seed per day. SUPERSEDES the 2026-05-22 v0.3.0 per-type pricing decision.** No price mechanism exists or is needed: photons = seeds makes the photon supply exactly the corpus, so the whole corpus can be rented at most once per day at full velocity — scarcity itself allocates. Buyers choose which seeds to spend on; differentiation across data types expresses as VOLUME (which seeds sell, and how often), never price; the photon's real-world value finds its own equilibrium at the edges. Creator equality becomes absolute — across creators AND across types. `MsgSetTypePrice`/`TypePrices` retire at the first governed upgrade; "the protocol never sets prices" is restored in its strongest form: there is no price to set.
- **2026-07-16 — ALL kinds mint: photons = distinct atoms, no gate.** "All atoms are observations. There is no data that can exist without it being an observation — that's a universal fact." The `mintable_kinds` gate (a pre-DT-18 batch_root artifact; its existence signaled build drift, not design) is removed at the first governed upgrade; every NEW leaf-seed mints regardless of kind, and the peg caveat in parameters.md v0.8.0 is void. A per-subject/day rate cap is noted as an optional future guard; `max_batch_new_count` suffices for now.
...
- **2026-07-17 — Observation IS attestation (owner clarification, recorded to stop a recurring drift).** "An observation made by gnosis is gnosis attesting to that data." There is ONE primitive — the signed claim — and the 2026-07-11 generalization ruling ("attestations are the universal relationship/event primitive") extends all the way down: a seed commit is the base-case attestation (of observation: I saw this, at this time, from this source), and rigor/use/replication/outcome are the same primitive at higher stakes. **x/seeds and x/attest are two LEDGERS of one primitive, not two primitives**: the seeds ledger conserves SUPPLY (photons = distinct observations; free entry, standing-blind by design — anyone or anything can observe, and observing does not make the data valuable), the attest ledger conserves STANDING (claims weighted by the claimant's reputation, judged by outcomes). An observation's evidentiary WEIGHT flows from the observer's standing and from convergence across independent observers — gnosis's millions of atoms are millions of observation-attestations whose value emerges through corroboration, not existence. Build-language rule: statements like "no attestations exist" must name the LEDGER (attest-ledger entries), never the primitive — the conflation caused real owner-facing miscommunication and is retired.
