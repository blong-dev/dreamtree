...
reputation:
  ...
  e_cap_mult: 2.0                     # RATIFIED shape 2026-07-17 (value INTERIM) — endorsement breadth cap: E_cap = e_cap_mult × max(e_i), paper-shape aggregate
...
| `reputation.e_cap_mult` | 2.0 | multiple | endorsement breadth cap (E_cap = mult × strongest single endorsement; paper-shape) | ≥ 1 | shape ratified; **value INTERIM** |
...
| `economics.value_creation_tax` | 0.005 | fraction | infrastructure funding from producer SALE revenue (ratified 2026-07-16: sale, not issuance) | [0, 1] | founder→governance |
...
- **2026-07-17 — v0.9.2 (backtest M1-M4 built).** The measurement instrument exists: projection decoupled behind LogReader/ReputationResolver (identical math on-chain and over exported logs); `citation_uplift_lambda` promoted const→governable param (rides upgrade-1); StrengthAt/WorkValueAt dial queries (params_override + as_of, read-only); `cmd/backtest` harness (walk-forward Spearman, independent-outcome separation, per-lever sensitivity curves, refuses to crown below a minimum outcome count). First real report waits on outcome attestations existing — the live log has none yet; reported honestly, not padded.
- **2026-07-17 — v0.9.1 (endorsement breadth bound).** Ratified (owner): endorsement contributions per (endorsee, domain) aggregate paper-shape — `E_cap·[1−Π(1−eᵢ/E_cap)]`, `E_cap = e_cap_mult × max(eᵢ)`. New lever `reputation.e_cap_mult = 2.0` (shape ratified, value INTERIM — backtest tunes). Closes the mass-endorsement (prep-school) hole; build change rides the first governed upgrade.
